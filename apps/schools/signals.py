...
def _school_post_save(sender, instance: School, created: bool, **kwargs):
    ...
    if changes:
        record_change(entity="schools.School", object_id=instance.pk, action="UPDATED", changes=changes)
...
def _invoice_pre_save(sender, instance: SubscriptionInvoice, **kwargs):
    if not instance.pk:
        instance._changelog_before = None
        return
    before = SubscriptionInvoice.objects.filter(pk=instance.pk).values(*INVOICE_WATCH_FIELDS).first()
    instance._changelog_before = before or None
...
def _invoice_post_save(sender, instance: SubscriptionInvoice, created: bool, **kwargs):
    if created:
        record_change(entity="schools.SubscriptionInvoice", object_id=instance.pk, action="CREATED", changes={})
        return
    before = getattr(instance, "_changelog_before", None) or {}
    ...
    for field in INVOICE_WATCH_FIELDS:
        if before.get(field) != after.get(field):
            changes[field] = {"before": before.get(field), "after": after.get(field)}
    if changes:
        record_change(entity="schools.SubscriptionInvoice", object_id=instance.pk, action="UPDATED", changes=changes)


@receiver(post_delete, sender=SubscriptionInvoice)
def _invoice_post_delete(sender, instance: SubscriptionInvoice, **kwargs):
    record_change(entity="schools.SubscriptionInvoice", object_id=instance.pk, action="DELETED", changes={})
...
def _payment_pre_save(sender, instance: SubscriptionPayment, **kwargs):
    if not instance.pk:
        instance._changelog_before = None
        return
    before = SubscriptionPayment.objects.filter(pk=instance.pk).values(*PAYMENT_WATCH_FIELDS).first()
    instance._changelog_before = before or None
...
def _payment_post_save(sender, instance: SubscriptionPayment, created: bool, **kwargs):
    if created:
        record_change(entity="schools.SubscriptionPayment", object_id=instance.pk, action="CREATED", changes={})
        return
    before = getattr(instance, "_changelog_before", None) or {}
    ...
    for field in PAYMENT_WATCH_FIELDS:
        if before.get(field) != after.get(field):
            changes[field] = {"before": before.get(field), "after": after.get(field)}
    if changes:
        record_change(entity="schools.SubscriptionPayment", object_id=instance.pk, action="UPDATED", changes=changes)


@receiver(post_delete, sender=SubscriptionPayment)
def _payment_post_delete(sender, instance: SubscriptionPayment, **kwargs):
    record_change(entity="schools.SubscriptionPayment", object_id=instance.pk, action="DELETED", changes={})
