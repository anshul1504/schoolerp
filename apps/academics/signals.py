...
def _ay_post(sender, instance: AcademicYear, created: bool, **kwargs):
    if created:
        record_change(
            entity="academics.AcademicYear", object_id=instance.pk, action="CREATED", changes={}
        )
        return
    changes = _diff(instance, ["school_id", "name", "start_date", "end_date", "is_current"])
    if changes:
        record_change(
            entity="academics.AcademicYear",
...
def _ay_del(sender, instance: AcademicYear, **kwargs):
    record_change(
        entity="academics.AcademicYear", object_id=instance.pk, action="DELETED", changes={}
    )
...
def _cm_post(sender, instance: ClassMaster, created: bool, **kwargs):
    if created:
        record_change(
            entity="academics.ClassMaster", object_id=instance.pk, action="CREATED", changes={}
        )
        return
    changes = _diff(instance, ["school_id", "name", "is_active"])
    if changes:
        record_change(
            entity="academics.ClassMaster", object_id=instance.pk, action="UPDATED", changes=changes
        )
...
def _cm_del(sender, instance: ClassMaster, **kwargs):
    record_change(
        entity="academics.ClassMaster", object_id=instance.pk, action="DELETED", changes={}
    )
...
def _sm_post(sender, instance: SectionMaster, created: bool, **kwargs):
    if created:
        record_change(
            entity="academics.SectionMaster", object_id=instance.pk, action="CREATED", changes={}
        )
        return
    changes = _diff(instance, ["school_id", "name", "is_active"])
    if changes:
        record_change(
            entity="academics.SectionMaster",
            object_id=instance.pk,
            action="UPDATED",
            changes=changes,
        )
...
def _sm_del(sender, instance: SectionMaster, **kwargs):
    record_change(
        entity="academics.SectionMaster", object_id=instance.pk, action="DELETED", changes={}
    )
...
def _sub_post(sender, instance: SubjectMaster, created: bool, **kwargs):
    if created:
        record_change(
            entity="academics.SubjectMaster", object_id=instance.pk, action="CREATED", changes={}
        )
        return
    changes = _diff(instance, ["school_id", "name", "is_active"])
    if changes:
        record_change(
            entity="academics.SubjectMaster",
            object_id=instance.pk,
            action="UPDATED",
            changes=changes,
        )
...
def _sub_del(sender, instance: SubjectMaster, **kwargs):
    record_change(
        entity="academics.SubjectMaster", object_id=instance.pk, action="DELETED", changes={}
    )
