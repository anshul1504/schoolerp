...
def build_frontoffice_dashboard_context(request, *, current_section="frontoffice"):
    ...
    enquiries = Enquiry.objects.select_related("school", "created_by", "converted_student").filter(school_id__in=allowed_school_ids)
    visitors = VisitorLog.objects.select_related("school", "created_by").filter(school_id__in=allowed_school_ids)
    calls = CallLog.objects.select_related("school", "created_by", "enquiry", "student").filter(school_id__in=allowed_school_ids)
    meetings = MeetingRequest.objects.select_related("school", "created_by", "principal", "enquiry").filter(school_id__in=allowed_school_ids)
    ...
    context.update(
        {
            ...
            "recent_follow_ups": EnquiryFollowUp.objects.filter(enquiry__in=enquiries)
            .select_related("enquiry", "created_by")[:8],
            "recent_calls": calls.order_by("-created_at")[:6],
            "upcoming_meetings": meetings.filter(status__in=["REQUESTED", "SCHEDULED"]).order_by("scheduled_at", "-created_at")[:6],
            ...
            "frontoffice_stats": {
                ...
                "overdue_follow_ups": enquiries.filter(follow_up_date__lt=today).exclude(status="CLOSED").count(),
...
@role_required("SUPER_ADMIN", "PRINCIPAL", "RECEPTIONIST")
def frontoffice_search(request):
    ...
    students = Student.objects.filter(school_id__in=school_ids, is_active=True).select_related("school")
    ...
    if q:
        ...
        enquiries = enquiries.filter(
            models.Q(student_name__icontains=q) | models.Q(guardian_name__icontains=q) | models.Q(phone__icontains=q)
        )
    else:
...
def frontoffice_daily_report(request):
    ...
    follow_ups_due_today = enquiries.filter(follow_up_date=today).order_by("follow_up_date", "-created_at")[:50]
    follow_ups_overdue = enquiries.filter(follow_up_date__lt=today).exclude(status="CLOSED").order_by("follow_up_date")[:50]
    ...
    # Calls snapshot
    calls_due_today = calls.filter(follow_up_date=today, status="OPEN").order_by("follow_up_date", "-created_at")[:50]
    calls_overdue = calls.filter(follow_up_date__lt=today, status="OPEN").order_by("follow_up_date")[:50]
    ...
    context.update(
        {
            ...
            "kpis": {
                ...
                "follow_ups_overdue": enquiries.filter(follow_up_date__lt=today).exclude(status="CLOSED").count(),
                "visitors_today": visitors.filter(entry_time__date=today).count(),
                "open_visitors": visitors.filter(exit_time__isnull=True).count(),
...
def enquiry_list(request):
    ...
    school = _selected_school(request)
    qs = Enquiry.objects.select_related("school", "created_by", "converted_student").filter(school_id__in=_school_ids_for_user(request.user))
    if school:
        qs = qs.filter(school=school)

    query = (request.GET.get("q") or "").strip()
    if query:
        qs = qs.filter(student_name__icontains=query) | qs.filter(guardian_name__icontains=query) | qs.filter(phone__icontains=query)
...
def enquiry_create_page(request):
    ...
    if request.method == "POST":
        school, error_redirect = _require_school(request, redirect_url="/frontoffice/enquiries/create/")
        if error_redirect:
...
def enquiry_edit(request, enquiry_id):
    ...
    enquiry = get_object_or_404(Enquiry, id=enquiry_id, school_id__in=_school_ids_for_user(request.user))
    if request.method == "POST":
        ...
        enquiry.interested_class = request.POST.get("interested_class", enquiry.interested_class).strip()
...
def followup_list(request):
    ...
    school = _selected_school(request)
    qs = EnquiryFollowUp.objects.select_related("enquiry", "enquiry__school", "created_by").filter(enquiry__school_id__in=_school_ids_for_user(request.user))
    if school:
...
def visitor_list(request):
    ...
    school = _selected_school(request)
    qs = VisitorLog.objects.select_related("school", "created_by").filter(school_id__in=_school_ids_for_user(request.user))
    if school:
        qs = qs.filter(school=school)
...
def visitor_create_page(request):
    ...
    if request.method == "POST":
        school, error_redirect = _require_school(request, redirect_url="/frontoffice/visitors/create/")
        if error_redirect:
            return error_redirect
...
def visitor_edit(request, visitor_id):
    ...
    visitor = get_object_or_404(VisitorLog, id=visitor_id, school_id__in=_school_ids_for_user(request.user))
    if request.method == "POST":
...
def call_list(request):
    if not has_permission(request.user, "frontoffice.view"):
        ...
        return redirect("dashboard")
    school = _selected_school(request)
    qs = CallLog.objects.select_related("school", "enquiry", "student", "created_by").filter(school_id__in=_school_ids_for_user(request.user))
    if school:
        qs = qs.filter(school=school)
...
def call_create(request):
    ...
    if request.method == "POST":
        ...
        CallLog.objects.create(
            school=school,
            enquiry_id=int(request.POST.get("enquiry_id")) if (request.POST.get("enquiry_id") or "").isdigit() else None,
            student_id=int(request.POST.get("student_id")) if (request.POST.get("student_id") or "").isdigit() else None,
    ...
    if enquiry_id.isdigit():
        enquiry = Enquiry.objects.filter(id=int(enquiry_id), school_id__in=_school_ids_for_user(request.user)).first()
        if enquiry:
...
def meeting_list(request):
    ...
    school = _selected_school(request)
    qs = MeetingRequest.objects.select_related("school", "principal", "enquiry", "created_by").filter(school_id__in=_school_ids_for_user(request.user))
    if school:
        qs = qs.filter(school=school)
...
def meeting_create(request):
    ...
    if request.method == "POST":
        school, error_redirect = _require_school(request, redirect_url="/frontoffice/meetings/create/")
        if error_redirect:
            return error_redirect
        ...
        if principal_id.isdigit():
            principal = User.objects.filter(id=int(principal_id), role="PRINCIPAL", school=school).first()
        ...
        if principal and principal.email:
            ...
            try:
                ...
                if comm and comm.smtp_enabled:
                    send_email_via_school_smtp(settings_obj=comm, to_email=principal.email, subject=subject, body=body)
                else:
                    EmailMultiAlternatives(subject=subject, body=body, to=[principal.email]).send(fail_silently=True)
            except Exception:
                pass
    ...
    school = _selected_school(request)
    principals = User.objects.filter(role="PRINCIPAL", school=school) if school else User.objects.none()
    context = build_layout_context(request.user, current_section="frontoffice")
    context.update(
        {
            "school_options": _school_scope(request.user),
...
def meeting_edit(request, meeting_id):
    ...
    meeting = get_object_or_404(MeetingRequest, id=meeting_id, school_id__in=_school_ids_for_user(request.user))
...
def template_create(request):
    ...
    if request.method == "POST":
        school, error_redirect = _require_school(request, redirect_url="/frontoffice/messages/templates/create/")
        if error_redirect:
            return error_redirect
...
def template_edit(request, template_id):
    ...
    template = get_object_or_404(MessageTemplate, id=template_id, school_id__in=_school_ids_for_user(request.user))
...
def campaign_list(request):
    if not has_permission(request.user, "frontoffice.view"):
        ...
        return redirect("dashboard")
    school = _selected_school(request)
    qs = MessageCampaign.objects.filter(school_id__in=_school_ids_for_user(request.user)).select_related("template", "created_by")
    if school:
        qs = qs.filter(school=school)
    context = build_layout_context(request.user, current_section="frontoffice")
    context.update(
...
def campaign_create(request):
    ...
    if request.method == "POST":
        school, error_redirect = _require_school(request, redirect_url="/frontoffice/messages/campaigns/create/")
        if error_redirect:
            return error_redirect
    ...
    if notice_id.isdigit():
        ...
        notice = Notice.objects.filter(id=int(notice_id), school_id__in=_school_ids_for_user(request.user), is_published=True).first()
        if notice:
            school = notice.school

    templates = MessageTemplate.objects.filter(school=school, is_active=True) if school else MessageTemplate.objects.none()
    prefill = {
...
def campaign_detail(request, campaign_id):
    if not has_permission(request.user, "frontoffice.view"):
        messages.error(request, "You do not have permission to view campaigns.")
        return redirect("dashboard")
    campaign = get_object_or_404(MessageCampaign, id=campaign_id, school_id__in=_school_ids_for_user(request.user))
            ...
            if action == "retry" and delivery.status in {"FAILED", "SKIPPED", "QUEUED"}:
                try:
                    if campaign.channel == "EMAIL" and delivery.recipient_contact:
                        ...
                        messages.info(request, "WhatsApp delivery remains queued until provider integration is configured.")
                    else:
...
def campaign_send(request, campaign_id):
    ...
    for label, email, phone in _recipients_for_campaign(campaign):
        if campaign.channel == "EMAIL":
            if not email:
                _log_delivery_event(MessageDeliveryLog.objects.create(
                    campaign=campaign,
                    channel="EMAIL",
                    ...
                    status="SKIPPED",
                    error="Missing email",
                ), status="SKIPPED", error="Missing email")
                skipped += 1
                continue
            try:
                comm = SchoolCommunicationSettings.objects.filter(school=campaign.school).first()
                if comm and comm.smtp_enabled:
                    ...
                    EmailMultiAlternatives(subject=campaign.subject or campaign.title, body=campaign.body, to=[email]).send(fail_silently=False)
                _log_delivery_event(MessageDeliveryLog.objects.create(
                    campaign=campaign,
                    channel="EMAIL",
                    recipient_label=label,
            ...
            _log_delivery_event(MessageDeliveryLog.objects.create(
                ...
                status="QUEUED",
                error="WhatsApp provider not configured (placeholder log).",
            ), status="QUEUED", error="WhatsApp provider not configured (placeholder log).")
            sent += 1

    campaign.status = "SENT"
    campaign.sent_at = timezone.now()
    campaign.save(update_fields=["status", "sent_at"])
    messages.success(request, f"Campaign processed: {sent} queued/sent, {skipped} skipped, {failed} failed.")
    return redirect(f"/frontoffice/messages/campaigns/{campaign.id}/")
...
def document_reminders(request):
    ...
    school = _selected_school(request)
    school_ids = _school_ids_for_user(request.user)
    students = Student.objects.filter(school_id__in=school_ids, is_active=True).order_by("first_name", "last_name")
    if school:
        students = students.filter(school=school)
    ...
    templates = MessageTemplate.objects.filter(school_id__in=school_ids, is_active=True, target="PARENTS")
    if school:
        templates = templates.filter(school=school)
    ...
    if request.method == "POST":
        ...
        school_for_send, error_redirect = _require_school(request, redirect_url="/frontoffice/document-reminders/")
        if error_redirect:
            return error_redirect
        ...
        template_id = (request.POST.get("template_id") or "").strip()
        if template_id.isdigit():
            template = MessageTemplate.objects.filter(id=int(template_id), school=school_for_send, is_active=True).first()
        if not template:
        ...
        skipped = 0
        failed = 0
        recipients = Student.objects.filter(school=school_for_send, is_active=True).order_by("id")[:500]
        for student in recipients:
            ...
            if channel == "EMAIL":
                if not email:
                    ...
                    continue
                try:
                    comm = SchoolCommunicationSettings.objects.filter(school=school_for_send).first()
                    if comm and comm.smtp_enabled:
                        send_email_via_school_smtp(settings_obj=comm, to_email=email, subject=subject, body=body)
                    else:
                        EmailMultiAlternatives(subject=subject, body=body, to=[email]).send(fail_silently=False)
                    _log_delivery_event(MessageDeliveryLog.objects.create(
                        campaign=campaign,
                        channel="EMAIL",
                        recipient_label=label,
        ...
        campaign.sent_at = timezone.now()
        campaign.save(update_fields=["status", "sent_at"])
        messages.success(request, f"Reminder processed: {sent} queued/sent, {skipped} skipped, {failed} failed.")
        return redirect(f"/frontoffice/messages/campaigns/{campaign.id}/")
...
def enquiry_update(request, enquiry_id):
    if not has_permission(request.user, "frontoffice.manage"):
        ...
        return redirect("frontoffice-overview")
    enquiry = get_object_or_404(Enquiry, id=enquiry_id, school_id__in=_school_scope(request.user).values("id"))
    if request.method != "POST":
        return redirect("frontoffice-overview")
    enquiry.status = request.POST.get("status", enquiry.status)
    enquiry.follow_up_date = request.POST.get("follow_up_date") or None
...
def enquiry_follow_up_create(request, enquiry_id):
    ...
    enquiry.notes = "\n".join(filter(None, [enquiry.notes.strip(), follow_up.summary.strip()])).strip()
...
def visitor_checkout(request, visitor_id):
    if not has_permission(request.user, "frontoffice.manage"):
        ...
        return redirect("frontoffice-overview")
    visitor = get_object_or_404(VisitorLog, id=visitor_id, school_id__in=_school_scope(request.user).values("id"))
